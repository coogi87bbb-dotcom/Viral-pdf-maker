...
// Prepended to every AI generation prompt across the whole app, per direct
// product instruction: every single generation should get the model's
// full "highly gifted genius" reasoning quality, not a cost/latency-capped
// version of it. Applied as a small text prefix (not a thinkingConfig cap)
// so it never reduces the model's own reasoning budget — see
// callGeminiWithResilience's maxOutputTokens comments for the companion
// fix (thinking-model token budgets raised so reasoning + full output both
// fit, discovered from a real "No content generated by AI." failure where
// thinking alone consumed nearly the entire old 1800-token budget).
const GENIUS_PERSONA_PREFIX =
  'Operate as a highly gifted genius in whatever role is described below — producing the absolute best possible output every single time, with total command of the subject matter and meticulous attention to detail. ';

// Speed fix, confirmed live: gemini-3.6-flash is a mandatory-thinking model
// (it rejects thinkingConfig.thinkingBudget: 0 with a 400 INVALID_ARGUMENT
// — thinking cannot be turned off on it at all) and its baseline latency
// with thinking averaged 15-20+ seconds per call (thoughtsTokenCount
// ranged ~1200-8800 across live test runs). Per direct product
// instruction ("take off extended thinking" — should not take longer than
// ~5-7s), switched every text-generation route from gemini-3.6-flash to
// gemini-3.1-flash-lite, which DOES accept thinkingBudget: 0. Quality is
// still driven by GENIUS_PERSONA_PREFIX (prompt text, not a thinking
// setting) and the same generous maxOutputTokens ceilings — only the
// internal extended-reasoning step is removed, not the persona framing or
// output length.
const FAST_MODEL = 'gemini-3.1-flash-lite';
const NO_THINKING_CONFIG = { thinkingConfig: { thinkingBudget: 0 } };

// 30mb (raised from 10mb): /api/docs/parse-file sends a base64-encoded
// uploaded document in a single JSON body — base64's ~33% inflation plus
// JSON envelope overhead means even a moderately large source file can
// approach the old 10mb ceiling. Global bump rather than a route-scoped
// override: Express applies body-parsing middleware once per request in
// registration order, so a route-specific
    ...
    const { response } = await callGeminiWithResilience(
      () =>
        ai.models.generateContent({
          ...
          // No GENIUS_PERSONA_PREFIX here deliberately — this is a literal
          // verbatim-transcription task, not creative generation; a
          // "genius" framing risks the model "improving"/paraphrasing
          // text instead of faithfully extracting it. Still needs an
          // explicit budget for the same reason as every other route
          // (thinking-model token exhaustion — see GENIUS_PERSONA_PREFIX's
          // comment), since a long source document could hit MAX_TOKENS.
          config: { ...NO_THINKING_CONFIG, maxOutputTokens: 32000 }
        }),
      'doc-parse-buffer'
    );
  ...
  try {
    ...
    try {
      ...
      const response = await ai.models.generateContent({
        model: FAST_MODEL,
        contents: `${GENIUS_PERSONA_PREFIX}${prompt}`,
        config: { ...NO_THINKING_CONFIG, maxOutputTokens: 32000 }
      });
...
- Break text into clear chapters with subtitles, graphic callout boxes (tips, key insights, action steps), visual bulletCards with badgeText labels, and structured data tables where appropriate.`;
    ...
    const prompt = `You are a highly gifted genius Digital Product Designer & Publishing Architect.
...
app.post('/api/ai/deal-closer-generate', async (req, res) => {
  try {
    ...
    // History (see FAST_MODEL/NO_THINKING_CONFIG's comment for the full
    // story): this route originally ran on gemini-3.6-flash with thinking
    // enabled, which caused two rounds of real, live-confirmed failures —
    // thinking silently consuming almost an entire small token budget and
    // truncating the answer ("No content generated by AI."), then, once
    // budgeted generously, taking 15-20+ seconds per call. Now on
    // FAST_MODEL with thinking fully disabled, so neither failure mode
    // applies — but the generous floor is left in place regardless (it's
    // harmless overhead now, not a truncation risk) rather than re-tuned
    // down, partly because several of the 7 Deal Closer tool components
    // still pass their own pre-existing literal (1800/2000/2400) as this
    // endpoint's maxTokens argument, which would silently override a plain
    // `|| 32000` fallback since those values are truthy — Math.max(...,
    // 32000) makes it a real floor regardless of what the caller sends.
    const clampedMaxTokens = Math.min(Math.max(Number(maxTokens) || 32000, 32000), 60000);

    // Single chokepoint — every one of the 7 Deal Closer tools proxies
    // through this route via callDealCloserAI.
    const geniusSystemPrompt = `${GENIUS_PERSONA_PREFIX}${systemPrompt}`;

    const { response: modelResponse, attempts } = await callGeminiWithResilience(
      () =>
        ai.models.generateContent({
          model: FAST_MODEL,
          contents: `${geniusSystemPrompt}\n\n${userPrompt}`,
          config: { ...NO_THINKING_CONFIG,
            temperature: 0.7,
...
// Contract Audit has no server route — it's a fully client-side tool (see
// src/components/ContractAudit/types.ts's header comment): runAudit
// (engine/auditEngine.ts) parses, matches, and classifies both uploaded
// CSVs entirely in the browser. This used to proxy to a separately-deployed
// FastAPI service; that bridge was removed once the variance-detection
// engine was ported natively.
...
app.post("/api/viral/generate-campaign", async (req, res) => {
  try {
    ...
    const prompt = `You are a highly gifted genius viral growth hacker and social media marketing director.
...
Make sure every single platform's output is EXTREMELY DETAILED, EXPANDED, AND PACKED with copy that feels 100% human-crafted and ready to publish immediately.`;
    ...
    const response = await ai.models.generateContent({
      ...
      config: { ...NO_THINKING_CONFIG,
        responseMimeType: "application/json",
        // Explicit generous cap — this prompt demands "5X COMPREHENSIVE"
        // detail across all 7 platforms, easily the largest expected
        // output in the app; without an explicit budget a thinking model
        // can silently truncate mid-JSON (see the shared genius-prefix
        // comment near GENIUS_PERSONA_PREFIX for how this was discovered).
        maxOutputTokens: 65536,
      },
    });
...
app.post("/api/viral/analyze", async (req, res) => {
  try {
    ...
    const prompt = `You are a highly gifted genius social media algorithm auditor, behavioral scientist, and virality score engine.
...
Return JSON object structured as:
{
  ...
  "segments": [
    ...
    const response = await ai.models.generateContent({
      model: FAST_MODEL,
      contents: `${GENIUS_PERSONA_PREFIX}${prompt}`,
      config: { ...NO_THINKING_CONFIG, responseMimeType: "application/json", maxOutputTokens: 32000 }
    });
...
app.post("/api/viral/competitor-analysis", async (req, res) => {
  try {
    ...
    const prompt = `You are a highly gifted genius Principal Software Architect and Competitive Intelligence Strategist.
...
app.post("/api/viral/generate-blueprint", async (req, res) => {
  try {
    ...
    const prompt = `You are a highly gifted genius Principal AI Systems Architect and Prompt Engineer.
...
app.post("/api/viral/test-blueprint", async (req, res) => {
  try {
    ...
    const response = await ai.models.generateContent({
      model: FAST_MODEL,
      // Deliberately NOT prefixed with GENIUS_PERSONA_PREFIX: this route
      // lets the user test their own custom system prompt (from the
      // blueprint generator above), so altering it would test something
      // other than what they actually typed. The fallback below (used only
      // when they haven't supplied one) still reflects the app's quality bar.
      contents: testInput,
      config: { ...NO_THINKING_CONFIG,
        systemInstruction: systemPrompt || `${GENIUS_PERSONA_PREFIX}You are a helpful AI assistant.`,
        maxOutputTokens: 32000
      }
    });
