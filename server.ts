...
app.post('/api/ai/deal-closer-generate', async (req, res) => {
  try {
    ...
    // gemini-3.6-flash is a thinking-enabled model — it spends part of the
    // token budget on internal reasoning before writing the actual answer
    // (confirmed live, twice: a 1800-token budget left only ~72 tokens for
    // real output after ~1724 went to thoughtsTokenCount; later, even with
    // the genius-persona framing's "meticulous" instruction pushing the
    // model into a longer self-verification reasoning pass, a 6000-token
    // budget still left only 239 tokens after thoughtsTokenCount hit 5756
    // — both truncated mid-answer with finishReason MAX_TOKENS, sometimes
    // with zero usable text, surfaced to the user as "No content generated
    // by AI."). Thinking-token usage is NOT deterministic — it varied
    // ~1400–5800 across otherwise-similar live test runs — so the budget
    // needs real margin above the worst case observed, not just enough to
    // pass one test. The fix is NOT to cap/disable thinking — per direct
    // product instruction, every generation should get the model's full
    // reasoning quality — it's to budget enough total tokens that thinking
    // AND the full written output both fit comfortably. 32000 is enforced
    // as a hard FLOOR, not just a default: several of the 7 Deal Closer
    // tool components still pass their own pre-thinking-model-era literal
    // (1800/2000/2400, tuned back when this model's reasoning overhead
    // wasn't a factor) as this endpoint's maxTokens argument, which would
    // silently override a plain `|| 32000` fallback since those values are
    // truthy. Math.max(..., 32000) ignores anything the caller asks for
    // below that floor; a caller can still ask for more, up to the 60000
    // ceiling (the model's own outputTokenLimit is 65536).
