...
app.post('/api/ai/deal-closer-generate', async (req, res) => {
  try {
    ...
    // (confirmed live: a 1800-token budget left only ~72 tokens for real
    // output after ~1724 went to thoughtsTokenCount, truncating mid-answer
    // with finishReason MAX_TOKENS and sometimes zero usable text at all,
    // surfaced to the user as "No content generated by AI."). The fix is
    // NOT to cap/disable thinking — per direct product instruction, every
    // generation should get the model's full reasoning quality — it's to
    // budget enough total tokens that thinking AND the full written
    // output both fit. 6000 default / 16000 ceiling (the model's own
    // outputTokenLimit is 65536, so there's large headroom).
    const clampedMaxTokens = Math.min(Math.max(Number(maxTokens) || 6000, 200), 16000);
