      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <MotionPanel3D tiltX={6} className="w-full" frosted={false}>
          <div className="bg-surface-1/30 backdrop-blur-xl backdrop-saturate-150 rounded-2xl border border-white/10 p-6 sm:p-8 shadow-xl">
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-6 sm:gap-8">
              {STATS.map((stat) => (
                <div key={stat.label} className="text-center space-y-1.5">
                  <div className="text-3xl sm:text-4xl font-black bg-gradient-to-r from-amber-300 via-amber-400 to-amber-200 bg-clip-text text-transparent">
                    {stat.value}
                  </div>
            ...
            <p className="text-center text-xs sm:text-sm text-slate-500 mt-6 pt-6 border-t border-slate-800">
