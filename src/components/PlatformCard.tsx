...
export const PlatformCard: React.FC<PlatformCardProps> = ({ content, onUpdateContent }) => {
  ...
  return (
    <MotionPanel3D tiltX={10} delay={0.1}>
        ...
        {isEditing ? (
          <div className="space-y-3">
            <textarea
              ...
              rows={12}
              className="w-full bg-slate-950 border border-violet-500/50 rounded-xl p-4 text-slate-100 text-sm focus:outline-none focus:ring-2 focus:ring-violet-500 font-mono leading-relaxed"
            />
          ...
          <div className="bg-slate-950 rounded-xl border border-slate-800 p-4 font-sans text-slate-200 text-sm leading-relaxed whitespace-pre-line shadow-inner max-h-[500px] overflow-y-auto">
            {content.mainBody}
          </div>
        )}
      </div>
      ...
      {content.platform === 'x' && (
        <div className="mt-4 p-4 bg-slate-950 rounded-xl border border-sky-500/30 space-y-3.5">
          ...
          <div className="space-y-3">
            {((content.mainBody || '').split(/\n\s*\n/).filter(p => p.trim().length > 0)).map((postText, idx) => {
              ...
              return (
                <div 
                  key={idx}
                  className={`p-3.5 rounded-xl bg-slate-900 border transition-all ${
                    isSafe ? 'border-slate-800 hover:border-sky-500/40' : 'border-amber-500/60 bg-amber-950/10'
                  }`}
                >
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-2">
                  ...
                  <div className="w-full h-1.5 bg-slate-950 rounded-full overflow-hidden mb-2">
                    <div 
                      className={`h-full transition-all ${
                        charLen <= 240 ? 'bg-sky-500' : charLen <= 280 ? 'bg-emerald-400' : 'bg-amber-500'
                      }`}
                      style={{ width: `${percent}%` }}
                    />
                  </div>
      ...
      {content.carouselSlides && content.carouselSlides.length > 0 && (
        <div className="mt-4 p-4 bg-slate-950 rounded-xl border border-pink-500/20 space-y-3">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-lg p-4 space-y-2">
            <div className="flex items-center justify-between text-xs text-pink-400 font-bold">
              ...
              <span className="text-slate-400 font-normal">{content.carouselSlides[activeSlide].visualConcept}</span>
            </div>
            <h4 className="text-base font-extrabold text-white">{content.carouselSlides[activeSlide].headline}</h4>
            <p className="text-xs text-slate-300 leading-relaxed">{content.carouselSlides[activeSlide].body}</p>
          </div>
      ...
      {content.scriptTiming && content.scriptTiming.length > 0 && (
        <div className="mt-4 p-4 bg-slate-950 rounded-xl border border-cyan-500/20 space-y-3">
              ...
              <tbody className="divide-y divide-slate-800/60">
                {content.scriptTiming.map((step, idx) => (
                  <tr key={idx} className="hover:bg-slate-900/50">
                    ...
                    <td className="py-2.5 px-3 text-slate-300">{step.visualCue}</td>
                    <td className="py-2.5 px-3 text-white font-medium">{step.audioVoiceover}</td>
                    <td className="py-2.5 px-3 text-amber-300 font-mono text-[11px]">{step.onScreenText}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      {content.discussionQuestions && content.discussionQuestions.length > 0 && (
        <div className="p-4 bg-slate-950 rounded-xl border border-blue-500/20 space-y-2">
      ...
      {(content.pinterestTitles || content.pinterestBoards) && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-xs">
          {content.pinterestTitles && content.pinterestTitles.length > 0 && (
            <div className="p-3 bg-slate-950 rounded-xl border border-red-500/20 space-y-1.5">
      ...
      {content.executiveTakeaways && content.executiveTakeaways.length > 0 && (
        <div className="p-4 bg-slate-950 rounded-xl border border-blue-400/20 space-y-2">
          <div className="text-xs font-bold text-blue-400 uppercase tracking-wide">
        ...
        {content.hashtags && content.hashtags.length > 0 && (
          <div className="flex flex-wrap items-center gap-1.5">
            {content.hashtags.map((tag, idx) => (
              <span key={idx} className="px-2 py-1 rounded bg-slate-800 text-violet-300 font-mono text-[11px]">
